...
const StoreDetailsCards = ({ data, dataFavorite, setStoreToApprove }: StoreCardsInterface) => {
    ...
    return (
        <Grid xs={12} container>
            {data?.map(item => (
                <Grid p={1} key={item.id} xs={getGridValue()} sx={{ maxHeight: '520px' }}>
                    <CardItem>
                        <Box
                            onClick={() => {
                        ...
                        >
                            <Box sx={{ width: '70%', borderRight: '1px solid #ccc' }}>
                                <Image store={{} as any} imgUrl={item?.mainImage as string} ref={null} />
                            </Box>
                            <Box
                                p={0.5}
...
export default StoreDetailsCards;
