...
const FindBySKUButton = ({ string, headerHeight }) => {
    ...
    return (
        <>
            <Box>
                <IconButton
                    sx={{ display: 'flex', flexDirection: 'column', '&:hover': { backgroundColor: '#fff' } }}
                    onClick={() => {
                        setShowSearch(!showSearch);
                    }}
                >
                    <SearchIcon />
                    <Typography sx={{ fontSize: 10, color: 'rgba(0, 0, 0, 0.54)' }}>{string?.search}</Typography>
                </IconButton>
            </Box>
            <Box
                sx={{
            ...
            >
                <Box
                    ...
                    sx={{
                        ...
                        overflow: 'hidden',
                        maxWidth: '500px',
                        borderBottom: showSearch ? '1px solid #ccc' : 'none',
                        backgroundColor: '#fff ',
                        ...
                        {showSearch && (
                            <>
                                <TextField
                                    ...
                                    sx={{ backgroundColor: '#fff', mr: 1, webkitUserSelect: 'input !important' }}
                                    fullWidth
                                    size="small"
                                ...
                                <IconButton
                                    ...
                                    size="small"
                                    sx={{ border: '1px solid #ccc', width: 32 }}
                                >
                    ...
                    >
                        {!!matchedSku.length ? (
                                ...
                                return (
                                    <Box key={idx} sx={{ display: showSearch ? 'block' : 'none' }}>
                                        <Box
                                            ...
                                            onClick={() => {
                                                navigate(
                                                    `/gallery/${item.productId}/details/${item.sku?.replaceAll(
                                                        '/',
                                                        '_'
                                                    )}`
                                                );
                            ...
                            <Box sx={{ display: showSearch ? 'block' : 'none' }}>
                                ...
                                >
                                    <Typography>{string?.no_results}</Typography>
                                </Box>
                            </Box>
                        )}
                    </Box>
...
export default FindBySKUButton;
